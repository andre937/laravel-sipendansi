                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-body">
                            @include('Admin.alert')
                            <button class="btn btn-sm btn-primary mb-3" data-toggle="modal" data-target="#tambah_kd"><i class="fa fa-plus fa-sm"></i> Tambah Data Nilai</button>
                            @if($nilais->count())
                                    ...
                                    <tbody>
                                        @foreach($nilais as $no => $nilai)
                                            <tr>
                                                ...
                                                <td>
                                                    <form action="{{ url('kd/'.$nilai->id) }}" class="d-inline" method="post" onsubmit="return confirm('apakah anda ingin menghapusnya ?')">
                                                        @method('delete')
                                                        @csrf
